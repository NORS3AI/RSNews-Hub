...
/**
 * Media picker for ad creatives and comics. On upload the file is sent to
 ...
 * Existing data-URL values keep working. Shows a live thumbnail. `kind="video"`
 * accepts silent mp4/webm creatives and previews them.
 */
export default function AdImageInput({
  name, label, hint, defaultValue = '', kind = 'image',
}: { name: string; label: string; hint?: string; defaultValue?: string; kind?: UploadKind }) {
      ...
      <div className="flex items-start gap-3">
        <div className="flex-1 space-y-2">
          <input value={value.startsWith('data:') ? '' : value} onChange={(e) => setValue(e.target.value)}
            placeholder={isVideo ? '/ads/spot.mp4 or https://…' : '/ads/your-banner.png or https://…'} className="input text-xs" />
          <div className="flex items-center gap-2">
            ...
            <input ref={fileRef} type="file" accept={isVideo ? 'video/mp4,video/webm' : 'image/*'} onChange={onPick} className="hidden" />
          </div>
        ...
        {value && (isVideo ? (
          ...
          <video src={value} muted loop playsInline autoPlay className="h-16 w-28 shrink-0 rounded-md border border-[var(--border)] object-contain" />
        ) : (
