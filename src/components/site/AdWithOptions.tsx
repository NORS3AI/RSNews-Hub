...
export default function AdWithOptions({
  ad, suppliers, savedIds, signedIn, slot, size = 'in-article', tone = 'card', fill = false, placeholder = true, adContext,
}: {
  ad: AdRow | null;
  ...
  if (!ad || !info) {
    return <InArticleAd ad={ad} slot={slot} size={size} tone={tone} fill={fill} placeholder={placeholder} adContext={adContext} />;
  }
  ...
  return (
    <div className={wrap}>
      <InArticleAd ad={ad} slot={slot} size={size} tone={tone} fill={fill} placeholder={placeholder} adContext={adContext} />
      <AdOptionsMenu
