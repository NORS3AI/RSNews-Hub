...
import { getActiveGenres } from '@/lib/genreServer';

export const dynamic = 'force-dynamic';

export default async function EditArticle(props: { params: Promise<{ id: string }> }) {
  const params = await props.params;
  const [article, categories, polls, quizzes, advertisers, reservedAds, revisions, reviews, vendors, pushTrail, bylines, genres] = await Promise.all([
  ...
  return (
    <>
      ...
      <ArticleEditor key={`${a.id}:${new Date(a.updatedAt).getTime()}:${a.draftSavedAt ? new Date(a.draftSavedAt).getTime() : 0}`}
        ...
        advertisers={advertisers} reservedAds={reservedAds} vendors={vendors} bylines={bylines} genres={genres.map((g) => ({ slug: g.slug, label: g.label }))} authorName={a.author?.name || 'You'} />
      <ArticleReviews
