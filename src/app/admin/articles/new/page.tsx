...
export default async function NewArticle() {
  const [categories, polls, quizzes, advertisers, reservedAds, vendors, bylines, genres, user] = await Promise.all([
  ...
  return <ArticleEditor categories={categories} polls={polls.map((p) => ({ id: p.id, title: p.question }))} quizzes={quizzes}
    advertisers={advertisers} reservedAds={reservedAds} vendors={vendors} bylines={bylines} genres={genres.map((g) => ({ slug: g.slug, label: g.label }))} authorName={user?.name || 'You'} />;
}
