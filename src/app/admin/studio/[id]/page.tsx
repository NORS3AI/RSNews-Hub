...
export default async function StudioEditorPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [mod, categories, advertisers, genres] = await Promise.all([
  ...
  if (!mod) notFound();
  return <StudioEditor id={mod.id} name={mod.name} published={mod.published} initialTree={parseTree(mod.tree)} categories={categories} advertisers={advertisers} genres={genres.map((g) => ({ slug: g.slug, label: g.label }))} />;
}
